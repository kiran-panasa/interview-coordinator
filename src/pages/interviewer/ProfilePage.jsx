...
const inputCls = "w-full border border-gray-200 rounded-xl px-3.5 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 transition-colors bg-white";
  ...
  return (
    <div className="p-8 max-w-2xl">
      ...
      <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.3 }}
        ...
        <div className="w-16 h-16 rounded-2xl bg-emerald-100 flex items-center justify-center flex-shrink-0 shadow-soft">
          <span className="text-2xl font-bold text-emerald-700">{initials}</span>
        </div>
        <div>
          <h1 className="text-2xl font-bold text-gray-900 tracking-tight">{form.displayName || "My Profile"}</h1>
          <p className="text-sm text-gray-400 mt-0.5">{userProfile?.email}</p>
        </div>
      </motion.div>

      <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.3, delay: 0.1 }}
        className="bg-white rounded-2xl border border-gray-100 shadow-soft divide-y divide-gray-100">
        ...
        {/* ── Save ── */}
        <div className="px-6 py-4 flex items-center justify-between bg-gray-50/60 rounded-b-2xl">
          <p className="text-xs text-gray-400">
            {changed ? "You have unsaved changes." : "All changes saved."}
          </p>
          <Button variant="primary" size="md" icon={saving ? undefined : Save}
